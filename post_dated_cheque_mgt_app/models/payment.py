...
class PdcAccountPayment(models.Model):
	...
	state = fields.Selection([('draft', 'Draft'), ('posted', 'Validated'), ('sent', 'Sent'), ('reconciled', 'Reconciled'), ('collect_cash','Collect Cash'), ('cancelled', 'Cancelled')], readonly=True, default='draft', copy=False, string="Status")
	...
	def _prepare_payment_moves(self):
		...
		for payment in self:
			...
			liquidity_line_account = payment.payment_type in ('outbound','transfer') and payment.journal_id.payment_debit_account_id.id or payment.journal_id.payment_credit_account_id.id
	...
	def collect_cash_button(self):
		...
		for payment in self:
			...
			if payment.payment_type in ('outbound', 'transfer'):
				counterpart_amount = payment.amount
				liquidity_line_account = payment.payment_type in ('outbound','transfer') and payment.journal_id.payment_debit_account_id.id or payment.journal_id.payment_debit_account_id.id
			else:
				counterpart_amount = payment.amount
				liquidity_line_account = pdc_account_id
			...
			if payment.payment_type == 'outbound':
				...
				counterpart_line_account = payment.payment_type in ('outbound','transfer') and payment.journal_id.payment_credit_account_id.id or payment.journal_id.payment_credit_account_id.id
